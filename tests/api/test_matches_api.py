...
class FakeMatchRepo:
    ...
    def count(self, company_id: int, *, clse_after) -> int:
        return len(_rows_for(company_id))

    def list_page(self, company_id: int, *, sort, limit, offset, clse_after):
        self.last_sort = sort   # 라우터가 넘긴 정렬키를 기록해 검증
        return _rows_for(company_id)[offset:offset + limit]
...
def test_matches_full_contract(match_client):
    ...
    assert body["total"] == 2 and body["page_size"] == PAGE_SIZE
    first = body["items"][0]
    # 중첩 구조: bid + match
